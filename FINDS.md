...
Every repo any discovery routine has ever surfaced. One entry per repo, appended
as it was found, newest at the top. Nothing here is scored, ranked or filtered:
if a sweep looked at it, it is in this file.

**573 entries · 2026-05-13 → 2026-08-16 · 855 distinct tags**
...
- **surfaced** is the date of the earliest sweep that mentioned the repo, not the
  date it was written up. Where no sweep file named it, the date is from the commit
  that first added it to a dedupe list.
- **alive** holds whatever liveness signal was on hand when the repo was captured —
  usually stars plus a created or last-active date. `not recorded` means the sweep
  never wrote one down. It is a gap in the record, not a finding about the repo.
- **why** keeps the original sweep's wording, including verdicts that were later
  reversed. Where a repo was reconsidered, both readings are in the text.
- **tags** are freeform. There is no controlled list and no fixed vocabulary — see
  the README on why that is deliberate.

Looking for a subset rather than the whole log? `views/` has the same entries as
scannable tables, filtered by tag.

## Sweeps

| date | entries first surfaced |
| --- | --- |
| 2026-08-16 | 1 |
| 2026-08-15 | 83 |
| 2026-08-13 | 7 |
...
Most-used tags: `audio` (340), `legal` (140), `mac-gaming` (82), `dedupe-only` (69), `no-commentary` (69), `unassessed` (69), `ableton` (61), `extensions-sdk` (42), `off-domain` (39), `hobbyist` (36), `midi` (31), `false-positive` (26).
...
alive:     thinner activity than frankea's fork
...
surfaced:  2026-07-01
what:      Surfaced by a sweep and logged to the dedupe list; no description was recorded.
alive:     not recorded
why:       Dedupe-list entry only — logged on 2026-07-01 so it would not resurface, but the sweep recorded no commentary. Carried across for completeness; the missing write-up is not a judgement on the repo. Re-triage from scratch if it resurfaces.
...
alive:     created 2026-06, mostly 0–25★ cohort
why:       One of ~45 hobbyist Extensions-SDK repos logged in bulk on 2026-07-01 — individually hobby-scale MIDI/notation/sample utilities, none a DSP-analysis tool. Real evidence the SDK ecosystem is alive; the SDK itself was the finding, not any one extension.
...
alive:     152 stars in 3 days; topics include `activation-tools`
...
alive:     created within the Extensions SDK's first month, three days apart from ableton-sdk-mcp
...
alive:     created within the Extensions SDK's first month (SDK beta opened 2026-06-02)
...
alive:     from 2012, not actively growing
...
alive:     created within the Extensions SDK's first month
...
what:      Hobbyist Ableton Extensions-SDK project from the ecosystem that sprang up in the SDK's first month.
alive:     created 2026-06, mostly 0–25★ cohort
why:       One of ~45 hobbyist Extensions-SDK repos logged in bulk on 2026-07-01 — individually hobby-scale MIDI/notation/sample utilities, none a DSP-analysis tool. Real evidence the SDK ecosystem is alive; the SDK itself was the finding, not any one extension. Same author as producer-pal.
tags:      audio, ableton, extensions-sdk, hobbyist, ecosystem
...
alive:     last real development 2018, stale
why:       Stale.
...
alive:     5179★; actively maintained
...
alive:     archived; no longer maintained
...
alive:     1912–1915★; Python/TF2; actively maintained through 2026; installable as a Python package
...
alive:     784–788★; Python/PyTorch; actively maintained 2026
...
alive:     1660–1663★; Python/Cython, Cython-optimized for throughput
...
what:      Surfaced by a sweep and logged to the dedupe list; no description was recorded.
alive:     not recorded
why:       Dedupe-list entry only — logged on 2026-06-10 so it would not resurface, but the sweep recorded no commentary. Carried across for completeness; the missing write-up is not a judgement on the repo. Re-triage from scratch if it resurfaces.
...
alive:     Python 2.7, last touched 2019; leans on the proprietary Melodia Vamp plugin
...
alive:     3★; one script, minimal docs
...
alive:     updated 2026-04; explicitly "no client-side audio processing"
...
alive:     Vue / cloud Supabase stack
...
alive:     runs entirely locally, no document content leaving the machine
...
alive:     an editor/host extension
...
alive:     runs locally via npm/Docker
...
alive:     weights on HF/ModelScope; runs via `infer.py` / Gradio / SGLang
...
alive:     Python + a TS desktop app; PyPI-packaged; v2.0.3
...
alive:     TypeScript; 43+ tests; v1.2.0
...
alive:     brand-new, 2026-05; TypeScript + Bun + Postgres + Redis; self-hostable via `bun run dev`; hosted preview at my.stll.app
...
alive:     2018 stub; 4★; barely maintained
...
alive:     research code rather than a packaged service
...
alive:     newer / quieter than Lumina-Note
...
alive:     v0.8.0, 2025-05; still maintained; works in CI via the documented Docker image
...
alive:     14★, under the sweep's 20-star bar
...
alive:     11★, under the sweep's 20-star bar
...
alive:     Spanish-leaning entity set today (DNI / NIG / ECLI)
...
alive:     TS/Python SDKs on the roadmap
...
alive:     weights on HF/ModelScope with SGLang/Transformers/Gradio inference
...
alive:     1★; documents running locally with Ollama
...
alive:     paper repo rather than a packaged model
...
alive:     82★; Python (+ Node) implementations
...
alive:     still getting commits; a 2017 project
...
alive:     ships a pretrained checkpoint
...
alive:     1.7k★; very active, active 2026-05
...
alive:     mostly a project page, README unreachable at capture
...
alive:     297★; still pushed in 2026 but last release 2019
...
alive:     286★; created 2024, active 2026-05
...
alive:     89★; created 2023, pushed within the 05-17 window; benchmarked ~476× realtime on M1
...
alive:     40★; created 2025-06; pre-alpha
...
alive:     25★; repo started 2012 but still pushed in 2026; README unreachable at one point
...
alive:     25–40★ (recorded inconsistently across sweeps); active 2026-04
...
alive:     21★; Python; active 2026-03
...
alive:     184★; Python; created 2026-05; active 2026-05; latency-tuned via Codex's `/goal`
...
alive:     200★; TypeScript; created 2025-08
...
alive:     27★; created 2026-01; inference model is binary-only
...
alive:     239★; Python; created 2025-02, active 2025-02
...
alive:     1.1k★; created 2026-03, active 2026-03
...
alive:     131★; created 2025-03; only a couple of filters shipped (LoButterworth, ParametricEQ)
...
alive:     268★; pushed within the 05-13 window
...
alive:     396★; created 2020 but still actively pushed in 2026
...
alive:     544★; pushed within the 05-13 window but the project itself is from 2019
...
alive:     superseded by the umbrella `gluon/Void-LinkAudio`
...
alive:     2015 project, recently touched
...
alive:     27★; created 2026-03; training-only, no weights
...
alive:     24★; TypeScript+Python; created 2026-01
...
alive:     146★; started 2021 but actively pushed
...
alive:     recent commits but no new direction
...
alive:     143★; JavaScript; active 2026-05
