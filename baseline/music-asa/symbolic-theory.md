...
Harmonia-adjacent stream. **Harmonia is real but unpublished** (a single-file,
dependency-free vanilla-JS chord/reharmonization tool, not on GitHub; *not* React/Tonal.js;
see `../README.md`): every `H` repo here is kept as a **conceptual reference only** (a
transferable algorithm / dataset / UX idea), not an incorporation target. Scores are **1–5
ASA-relevance** + flag (`ASA` · `H` · `Both` · `tang`), where `H` is an idea-only tag, not a
stack-fit score; corrected scores follow `discoveries/reanalysis-2026-05-20.md`.
...
- **4/H** · [madderscientist/noteDigger](https://github.com/madderscientist/noteDigger) · `JavaScript` · `268★` · `maturity:app`
  "No framework, no library" pure-JS audio→MIDI transcription: optimised real-FFT STFT, CQT, ONNX nnls.js + spectral-clustering note picking, harmonic reduction, beat tracking. (0% ASA-relevant — client-side JS.) **Mine:** Harmonia is symbolic-first and ingests no audio, so the transcription itself is only a loose *conceptual* reference; the genuinely transferable idea is its dependency-free, no-build pure-JS construction — the very constraint Harmonia is built under. _(surfaced 05-13 PM · tags: transcription, midi, cqt, onnx)_
...
- **3/H** · [chromatone/chromatone.center](https://github.com/chromatone/chromatone.center) · `146★` · `maturity:app`
  Vue/Vite app on Tonal.js + abcjs + Tone.js + audiomotion-analyzer presenting chords, scales, rhythms, and pitch-color visualisations as PWA "instruments". **Mine:** a *conceptual* reference for the chord/scale visual idioms and color-coded pitch palette only — Harmonia hand-rolls its own UI with no libraries, so this is an idea source, not a stack to match. _(surfaced 05-13 PM · tags: theory, tonaljs, scales, pwa)_
...
- **3/H** · [CPJKU/partitura](https://github.com/CPJKU/partitura) · `Python` · `350★` · `maturity:lib`
  Symbolic-score library across MusicXML, MIDI, Humdrum **kern**, and MEI, exposing notes (pitch/duration/voice/staff), parts, time signatures, beat maps. Off-stack (Python) but the cleanest complete symbolic data model. **Mine:** a *conceptual* reference for a richer score import/export model than a hand-rolled note table — copy its note/part/timeline model if Harmonia ever needs MusicXML/MEI/kern. _(surfaced 05-19 · tags: symbolic, musicxml, mei, kern)_
- **3/H** · [sivabenepoivediamo/musicplusplus](https://github.com/sivabenepoivediamo/musicplusplus) · `C++` · `maturity:lib`
  Header-only C++ music-theory library using vector-based representations for chords, scales, intervals, **voice leading**, and **reharmonization** (modal interchange, modulation); TypeScript and Python SDKs on the roadmap. Dead-center on Harmonia's *domain* — its Substitutions panel does exactly this. **Mine:** a *conceptual* algorithm reference for voice-leading + reharmonization to re-implement in Harmonia's own vanilla JS — not a dependency (Harmonia takes none). _(surfaced 05-21 · tags: theory, reharmonization, voice-leading, cpp)_
...
- **3/H** · [comorebi-notes/rechord](https://github.com/comorebi-notes/rechord) · `maturity:app`
  React + Tone.js app for writing and sharing chord progressions, still getting commits; a 2017 sharing app with no reharmonization logic. **Mine:** a *conceptual* reference for progression-entry UX only — nothing on the theory side, and the React/Tone.js stack does not transfer to Harmonia's dependency-free vanilla JS. _(surfaced 05-21 · tags: react, tonejs, progressions, ui)_
...
- **low/H** · [albertms10/music_notes](https://github.com/albertms10/music_notes) · `Dart` · `maturity:lib`
  Music-theory library in Dart. **Mine:** a *conceptual* reference for a clean theory type-model (intervals/keys/scales); the language is irrelevant since Harmonia would re-implement rather than import. _(surfaced 05-17 · tags: theory, dart, lib)_
