...
feeds ASA's tonal stage (and, as conceptual references only, Harmonia's chord/key
ideas). Scores are **1–5 ASA-incorporation relevance** plus a flag: `ASA` · `H`
(Harmonia = **conceptual reference only**: a real but unpublished single-file
vanilla-JS tool with no repo/deps, so `H` repos are kept as chord/key idea
references — mineable, never a fork or dependency target) · `Both` · `tang`. See
`../README.md` for the spec; corrected scores follow
`discoveries/reanalysis-2026-05-20.md`. Note: any "audio→Harmonia" / "input Harmonia
consumes" framing in entries below predates the correction — Harmonia is symbolic-first
and takes no audio input, so read those as ASA-side value only.
...
- **3/H** · [markwilkins/midi-chord-reader](https://github.com/markwilkins/midi-chord-reader) · `C++` · `22★` · `maturity:app`
  JUCE/C++ DAW plugin naming chords from a MIDI track during playback — normalises to one octave while preserving bass, generates slash inversions (`Am/C`), filters 2nd/4th/6th passing tones, uses the lowest three notes for quality. **Mine:** the chord-naming heuristics (octave-normalise + slash inversion + passing-tone filter) are a conceptual reference for Harmonia, which hand-rolls its own "given these notes, name the chord" logic with no theory library. _(surfaced 05-17 · tags: chord, midi, juce, heuristic)_

## Marginal — kept with a note (low)

- **low/H** · [lorediggia/harmony-lab](https://github.com/lorediggia/harmony-lab) · `Rust` · `maturity:app`
  Minimal Rust scale/chord explorer. Dropped 05-21 as too small to be a useful conceptual reference. **Mine:** only as a tiny reference for representing scales/chords in Rust if a native harmonic helper is wanted. _(surfaced 05-21 · tags: chord, scale, rust)_
- **low/tang** · [sepandhaghighi/capo](https://github.com/sepandhaghighi/capo) · `Python` · `maturity:lib`
  Python guitar-chord transposition. Dropped 05-21 (transposition is trivial — Harmonia already hand-rolls its own). **Mine:** capo/transpose mapping logic only, if a guitar-specific transpose ever comes up. _(surfaced 05-21 · tags: chord, guitar, transpose)_
