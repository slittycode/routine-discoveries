...
reharmonization tool (hand-rolled theory + a byte-level MIDI writer; **no React, no
Tonal.js, no build**) — **not on GitHub** (`github.com/slittycode/harmonia` → 404), no repo,
no deps, no backlog. (An earlier ASA-side correction over-reached by calling it a "phantom";
it is real, just unpublished and previously mis-described as React/Tonal.js.) With no codebase
to fork into and no stack to match, Harmonia is **not a scored incorporation target**: every
`H` repo is kept as a **conceptual chord/key/theory/UI reference** (idea-mining only), not a
fork or dependency target. See `../README.md` for the spec.

## Legend
- **Score = 1–5 ASA-incorporation relevance** plus a flag: `ASA` · `H` (Harmonia =
  **conceptual reference only**; unpublished single-file vanilla-JS tool, no repo/deps) ·
  `Both` · `tang` (tangential). Older 1–5 `H` numbers are retained as historical relevance,
  not stack-fit scores. This is the music schema — *not* the legal `fork/spark` scale.
...
- **`chord-key.md`** — chord and key detection (audio→chord, audio→key) and the harmonic-core MIR feeding ASA's tonal stage (and, as conceptual references only, Harmonia's chord/key ideas).
- **`symbolic-theory.md`** — symbolic music, music-theory libraries, datasets, and tokenizers — the Harmonia-conceptual-reference stream.
