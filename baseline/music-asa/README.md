...
so native C/C++/Rust/Python is first-class here. **Harmonia is a real, unpublished, single-file** vanilla-JS chord/reharmonization tool (no
React, no Tonal.js, no deps, no repo — `github.com/slittycode/harmonia` → 404); `H` repos are
**conceptual** chord/theory/UX idea references, not validated for stack-fit. See `../README.md` for the spec.

## Legend
- **Score = 1–5 relevance** to ASA/Harmonia plus a flag: `ASA` · `H` (Harmonia =
  **conceptual reference**; see framing above) · `Both` · `tang` (tangential). This is the music schema — *not* the
...
- **`chord-key.md`** — chord and key detection (audio→chord, audio→key) and the harmonic-core MIR feeding ASA's tonal stage and Harmonia as a conceptual consumer.
