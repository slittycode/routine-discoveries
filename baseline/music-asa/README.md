...
so native C/C++/Rust/Python is first-class here. **Harmonia correction:** it was
mis-described as "React + Tonal.js" in prior sweeps — it is a **real, unpublished,
dependency-free, vanilla-JS single-file** chord/reharmonization tool with no GitHub repo
(`github.com/slittycode/harmonia` → 404) and no npm ecosystem. Every `H` repo is a
**conceptual reference** for chord/theory/UI work — mineable for ideas, but
stack-compatibility scoring and "incorporate into Harmonia" framing do not apply.
See `../README.md` for the spec and `routines/audio-mir.md` for the authoritative
Harmonia description.

## Legend
- **Score = 1–5 relevance** to ASA/Harmonia plus a flag: `ASA` · `H` (Harmonia =
  **conceptual reference**, vanilla-JS single-file, unpublished; prior "React+Tonal.js"
  description was wrong) · `Both` · `tang` (tangential). This is the music schema —
  *not* the legal `fork/spark` scale.
...
- **`chord-key.md`** — chord and key detection (audio→chord, audio→key) and the harmonic-core MIR feeding ASA's tonal stage and Harmonia as a conceptual consumer (vanilla-JS single-file tool, not a React project).
