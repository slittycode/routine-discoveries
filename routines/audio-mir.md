...
music theory / music generation / LLM-assisted music tooling for the **ASA** and
**Harmonia** projects. One of two streams in this repo (see `routines/README.md`).

> Framing history: an earlier version called ASA "an in-browser Essentia.js library"
> and described **Harmonia** as a discoverable "React + Tonal.js" GitHub repo scored
> for fork / stack-fit. Both were wrong. ASA is a SERVER-SIDE Python app (below).
> **Harmonia is real but UNPUBLISHED** — a single local HTML file, not on GitHub
> (`github.com/slittycode/harmonia` → 404), so it is **not a discoverable/forkable
> repo and is not scored as one**. It remains a target only as a **conceptual
> reference**: repos can be surfaced as UX / dataset / algorithm ideas for it, never as
> forks, dependencies, or stack-fit incorporations. (Do **not** describe Harmonia as
> nonexistent or a "no-such-repo" target — it exists; it simply isn't published.)
...
## Project: Harmonia  (conceptual-reference target only)

Harmonia is a real but **unpublished single local HTML file**: a bare-bones "Chord
Progression Studio" in **vanilla HTML/CSS/JS with zero dependencies** (hand-rolled
music theory + a byte-level MIDI writer — **no React, no Tonal.js, no imports**).
Mood/genre → diatonic progressions, roman-numeral analysis, SVG piano, a chord
**Substitutions** panel (reharmonization), Web Audio playback, MIDI export.
Symbolic-first (not audio-first). **Not on GitHub** (404). Having no repo, build, or
dependencies, it **cannot be scored for incorporation** — surface repos that are
useful **conceptual references** for it (chord/theory UX, datasets, algorithms) only.
...
2. Search GitHub, filtered to created:>2025-01-01 OR pushed in the last ~14 days,
   ...
   - "chord progression" OR "music theory" OR "reharmonization"          # Harmonia conceptual refs
...
5. Score each 1–5 on **ASA** relevance. Drop anything <3 (unless it's a strong
   **Harmonia conceptual reference** — see the last bullet). Score ASA against the REAL
   stack:
   ...
   - Chord/key repos score on ASA relevance (they feed ASA's chord/key detection
     stage), flagged `ASA`.
   - **Harmonia is NOT scored as a repo** (it's unpublished, dependency-free — no stack
     to fit). Symbolic / MIDI / score-theory / chord-progression repos that don't serve
     ASA can still be surfaced as **Harmonia conceptual references** (`H`) when they
     offer a genuinely useful UX, dataset, or algorithm idea — flag `H`, give the ASA
     score, and don't assign an incorporation score. Drop the rest regardless of
     language.

6. Append survivors to `discoveries/audio-mir-<YYYY-MM-DD>.md`, sections:
   **ASA-relevant** / **Harmonia (conceptual references)** / **Both** / **Tangential
   but interesting**, plus a **Dropped** list with one-line reasons. Two-sentence pitch
   each, with link, ASA score, and flag — `ASA`, `H` (conceptual ref), `Both`, or
   `tang`. Harmonia entries are conceptual-only, never scored as repos.
